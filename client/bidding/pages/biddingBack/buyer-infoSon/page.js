...
$(() => {
    var vm = new Vue({
        ...
        methods: {
            getSon(currentPage) {
                ...
                $.ajaxForJson(config.wwwPath + "/subAccountPost", {
                    company_name: key,
                    role_type: type,
    ...
    $(".btn_confirm").unbind().bind("click", function () {
        var key = $.trim($("input[name='company_name']").val()), type = $("input[name='role_type']").val();
        $(".page_container_wrap").ajxForPage(config.wwwPath + "/subAccountPost", {
            company_name: key,
            role_type: type,
            status: $("select[name='status']").val(),
            join_way: $("select[name='join_way']").val()
    ...
    $(".back_main").on("click", ".back_title_btn,.change_status", function () {
        ...
        if ($(this).hasClass("change_status")) {
            ...
            dialogHTML = '<div class="son_account"><div class="son_account_form"><form action="' + config.wwwPath + 'editCompanyRole" id="addSon"><input type="hidden" name="id" value="' + id + '"><input type="hidden" name="account_type" value="' + obj.account_type + '"><input type="hidden" name="user_item_id" value="' + obj.user_item_id + '">';
            ...
            dialogHTML = '<div class="son_account"><div class="son_account_form"><form action="' + config.wwwPath + 'addSubaccount" id="addSon">';
            dialogHTML += '<div class="son_account_item"><label><span>*</span>姓名：</label><div class="son_account_block"><input type="text" name="real_name" class="cjy-input-" placeholder="请填写真实姓名"></div></div>';
            dialogHTML += '<div class="son_account_item"><label><span>*</span>手机：</label><div class="son_account_block"><input type="text" name="user_mobile" class="cjy-input-" placeholder="请输入手机号码，将作为登录帐号"></div></div>';
            dialogHTML += '<div class="son_account_item"><label><span>*</span>密码：</label><div class="son_account_block"><input type="password" name="user_pwd" class="cjy-input-" placeholder="8~20位数字及字母的组合"></div></div>';
            dialogHTML += '<div class="son_account_item"><label><span>*</span>邮箱：</label><div class="son_account_block"><input type="text" name="user_email" class="cjy-input-" placeholder="请输入邮箱，用于接收通知信息"></div></div>';
        ...
        $.dialog({
            ...
            callback: function () {
                ...
                $.ajaxForJson(config.wwwPath + "/itemLists", null, function (dataObj) {
                    if (dataObj.code == "2000") {
                        var liHTML = '';
