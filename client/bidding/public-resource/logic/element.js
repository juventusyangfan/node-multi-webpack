...
$(() => {
    $.fn.extend({
            ...
            $(this).each(function () {
                ...
                if (cityArr.length > 0) {
                    for (var i = 0; i < cityArr.length; i++) {
                        _cellHTML += '<span class="cjy-multiCity-cell" target_id="' + cityArr[i].id + '">' + cityArr[i].name + '<input type="hidden" name="scope_operation[]" value="' + cityArr[i].id + '"><i class="iconfont icon-cha1"></i></span>';
                    }
                } else {
                ...
                multi.find(".cjy-multi-city").off().on("click", "li a", function () {
                    ...
                    if (main.parent().hasClass("act")) {
                        ...
                        main.parent().addClass("act");
                        var _html = '<span class="cjy-multiCity-cell" target_id="' + id + '">' + name + '<input type="hidden" name="scope_operation[]" value="' + id + '"><i class="iconfont icon-cha1"></i></span>';
                        if (multi.find(".cjy-multi-wrap .cjy-multiCity-cell").length > 0) {
        ...
        //城市下拉控件
        initCitySelect: function (callback) {
            $(this).each(function () {
                var multi = $(this);

                var _provinceId = multi.attr("province-id") || "", //初始化时的省市区
                    _cityId = multi.attr("city-id") || "",
                    _provinceName = multi.attr("province-name") || "",
                    _cityName = multi.attr("city-name") || "";

                var multiHTML = '<div class="cjy-multi"><div class="cjy-multi-title"><input type="hidden" name="province_code" value="' + _provinceId + '"><input type="hidden" name="city_code" value="' + _cityId + '"><input type="hidden" name="province_name" value="' + _provinceName + '"><input type="text" name="city_name" placeholder="所在地区" readonly="" class="cjy-multi-input" value="' + _cityName + '"><i class="iconfont icon-xiajiantou"></i></div><div class="cjy-multi-content" style="width:400px;min-width:400px;"><ul class="cjy-multi-province"></ul><ul class="cjy-multi-city"></ul></div></div>';
                multi.html(multiHTML);
                $.ajaxJSONP(config.papiPath + "/api/common/getProvince", null, function (dataObj) {
                    if (dataObj.code == "2000") {
                        var liHTML = "";
                ...
                multi.find(".cjy-multi-city").off().on("click", "li a", function () {
                    var main = $(this);
                    var provinceId = multi.find(".cjy-multi-province").find("li.act a").attr("data-id"),
                        provinceName = multi.find(".cjy-multi-province").find("li.act a").attr("title"),
                        id = main.attr("data-id"),
                        name = main.attr("title");
                    multi.find(".cjy-multi-city").find("li").removeClass("act");
                    main.parent().addClass("act");
                    multi.find(".cjy-multi-input").val(name);
                    multi.find("input[name='city_code']").val(id);
                    multi.find("input[name='province_code']").val(provinceId);
                    multi.find("input[name='province_name']").val(provinceName);
                    multi.find(".cjy-multi").removeClass("cjy-selected");
                });
            });
        },
