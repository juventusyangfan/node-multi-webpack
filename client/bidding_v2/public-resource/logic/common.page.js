            ...
            _addEvent: function _addEvent() {
                ...
                DOM.wrap.bind('click', function (evt) {
                    ...
                    if (target[expando + 'D']) {
                        ...
                        that._draw(new Date(that.year, that.month - 1), parseInt(target[expando + 'D']));
                    } else if (target === DOM.pm[0] || target.parentNode === DOM.pm[0]) {
                        that._draw(new Date(that.year, that.month - 2), that.day);
                    } else if (target === DOM.nm[0] || target.parentNode === DOM.nm[0]) {
                        that._draw(new Date(that.year, that.month), that.day);
                    } else if (target === DOM.py[0] || target.parentNode === DOM.py[0]) {
                        that._draw(new Date(that.year - 1, that.month - 1), that.day);
                    } else if (target === DOM.ny[0] || target.parentNode === DOM.ny[0]) {
                        that._draw(new Date(that.year + 1, that.month - 1), that.day);
                    }
