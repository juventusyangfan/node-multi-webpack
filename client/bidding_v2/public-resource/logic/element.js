    ...
    $.fn.extend({
        ...
        initSelect: function () {
            $(this).each(function () {
                ...
                var readOnly = typeof (select.attr("cjy-search")) == "undefined" ? "readOnly" : "",
                    ...
                    placeholder = typeof (select.attr("placeholder")) == "undefined" ? select.find("option").eq(0).html() : select.attr("placeholder");
                text = readOnly == "" ? text == "请选择" ? "" : text : text;
                var selHTML = '<dd class="cjy-select" style="width: ' + selW + 'px;"><div class="cjy-select-title"><input type="text" placeholder="' + placeholder + '" value="' + text + '" ' + readOnly + ' class="cjy-select-input" style="width: ' + (selW - 40) + 'px;' + bgStyle + '"><i class="iconfont icon-xiajiantou"></i></div>';

                selHTML += '<dl style="width: ' + selW + 'px;">';

                if (typeof (select.attr("cjy-search-input")) != "undefined") {
                    selHTML += '<dd class="cjy-search-input" style="width: ' + (selW - 40) + 'px;"><input type="text" style="width: ' + (selW - 60) + 'px;"></dd><div class="cjy-search-input-list">';
                }

                for (var i = 0; i < select.find("option").length; i++) {
                    let className = val == select.find("option").eq(i).attr("value") || val == select.find("option").eq(i).attr("p-value") ? "sel-this" : "";
                    let disClass = select.find("option").eq(i).prop("disabled") ? 'cjy-disabled' : '';
                    if (typeof (select.find("option").eq(i).attr("p-value")) == "undefined") {
                        ...
                        selHTML += '<dd data-value="' + select.find("option").eq(i).attr("value") + '" p-value="' + select.find("option").eq(i).attr("p-value") + '" class="' + className + ' ' + disClass + '" title="' + select.find("option").eq(i).html() + '" style="width: ' + (selW - 40) + 'px;padding-left:30px;">' + select.find("option").eq(i).html() + '</dd>';
                    }
                }
                if (typeof (select.attr("cjy-search-input")) != "undefined") {
                    selHTML += '</div>';
                ...
                select.next(".cjy-select").find("dd").unbind().bind("click", function () {
                    var main = $(this);
                    if (!main.hasClass("cjy-search-input")) {
                        ...
                        main.parent().find("dd[p-value='" + opVal + "']").addClass("sel-this");
                        main.parents(".cjy-select").find(".cjy-select-input").val(opTxt);
                        select.val(opVal);
                        main.parents(".cjy-select").removeClass("cjy-selected");
                        select.trigger("change");
                        if (select.next(".cjy-select").next("input[name='" + select.attr("input-name") + "']").length <= 0) {
                            select.next(".cjy-select").after('<input type="hidden" name="' + select.attr("input-name") + '" value="' + opTxt + '">');
                        } else {
                            select.next(".cjy-select").next("input[name='" + select.attr("input-name") + "']").val(opTxt);
                        }
                    }
                });
                if (readOnly == "") {
                    select.next(".cjy-select").find(".cjy-select-input").unbind().bind("input", function () {
                        ...
                        if (select.next(".cjy-select").next("input[name='" + select.attr("input-name") + "']").length <= 0) {
                            select.next(".cjy-select").after('<input type="hidden" name="' + select.attr("input-name") + '" value="' + searchVal + '">');
                        } else {
                            select.next(".cjy-select").next("input[name='" + select.attr("input-name") + "']").val(searchVal);
                        }
                        $(".cjy-select").removeClass("cjy-selected");
                        main.parents(".cjy-select").addClass("cjy-selected");
                        let newSel = '';
                        for (var i = 0; i < select.find("option").length; i++) {
                            ...
                            if (select.find("option").eq(i).html().indexOf(searchVal) > -1) {
                                if (typeof (select.find("option").eq(i).attr("p-value")) == "undefined") {
                                    ...
                                    newSel += '<dd data-value="' + select.find("option").eq(i).attr("value") + '" p-value="' + select.find("option").eq(i).attr("p-value") + '" class="' + disClass + '" title="' + select.find("option").eq(i).html() + '" style="width: ' + (selW - 40) + 'px;padding-left:30px;">' + select.find("option").eq(i).html() + '</dd>';
                                }
                            }
                        }
                ...
                if (typeof (select.attr("cjy-search-input")) != "undefined") {
                    select.next(".cjy-select").find(".cjy-search-input input").unbind().bind("input", function () {
                        var main = $(this);
                        var searchVal = main.val();
                        if (select.next(".cjy-select").next("input[name='" + select.attr("input-name") + "']").length <= 0) {
                            select.next(".cjy-select").after('<input type="hidden" name="' + select.attr("input-name") + '" value="' + searchVal + '">');
                        ...
                        main.parents(".cjy-select").find("dl .cjy-search-input-list").html(newSel);
                        select.val("0");

                        select.next(".cjy-select").find("dl .cjy-search-input-list dd").unbind().bind("click", function () {
                            var main = $(this);
                            if (main.hasClass("cjy-disabled")) {
                                return;
                            }
                            ...
                            main.parent().find("dd[p-value='" + opVal + "']").addClass("sel-this");
                            main.parents(".cjy-select").find(".cjy-select-input").val(opTxt);
                            main.parents(".cjy-select").find(".cjy-search-input input").val("");
                            select.val(opVal);
                            main.parents(".cjy-select").removeClass("cjy-selected");
                            select.trigger("change");
                            if (select.next(".cjy-select").next("input[name='" + select.attr("input-name") + "']").length <= 0) {
        ...
        initCalendar: function (option) {
            $(this).each(function () {
                var cInput = $(this);
                var _option = null;
                if (cInput.attr("format")) {
                    _option = {
                        format: cInput.attr("format")
                    }
                }
                if (option) {
                    _option = option
                ...
                cInput.next(".cjy-calendar").find(".cjy-calendar-input").calendar(_option);
                cInput.remove();
            });
        }
    });
