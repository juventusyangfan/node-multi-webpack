...
$(() => {
    ...
    $(".page_list").on("click", ".js_choose_dialog", function () {
        ...
        html += '<table><colgroup><col width="5%"><col width="8%"><col width="18%"><col width="8%"><col width="12%"><col width="15%">';
        if (_proType == "3") {
            html += '<col width="15%">';
        }
        html += '<col width="10%"><col width="15%"></colgroup>';
        html += '<thead><tr><th>排序</th><th>候选人排名</th><th>候选人</th><th>评标得分</th><th>报价</th>';
        if (_proType == "3") {
            html += '<th>确认报价(钢筋类)</th><th>确认报价(非钢筋类)</th>';
        } else {
            html += '<th>确认报价</th>';
        }
        html += '<th>选择</th><th>排序</th></tr></thead><tbody>';
        html += '<tr v-for="(item,index) in result"><td>{{index + 1}}</td><td>【{{item.idx}}】</td><td>{{item.seller_company_name}}</td><td>{{item.evaluate_score}}</td><td>{{item.synthetic_price}}元</td>';
        if (_proType == "3") {
                ...
                var vm = new Vue({
                    ...
                    methods: {
                        getBidList() {
                            var that = this;
                            if (!main.attr("result")) {
                                $.ajaxForJson(config.wwwPath + "buyer/Tender/getCandidateAjax", {
                ...
                $(".cjy-confirm-btn").unbind().bind("click", function () {
                    ...
                    for (var i = 0; i < vm.result.length; i++) {
                        if (vm.result[i].checked) {
                            _html += '<p apply_id="' + vm.result[i].apply_id + '" seller_company_id="' + vm.result[i].seller_company_id + '" evaluate_score="' + vm.result[i].evaluate_score + '" quote_price="' + vm.result[i].synthetic_price + '" confirm_quote_price="' + vm.result[i].confirm_price + '" confirm_quote_gj_price="' + vm.result[i].gj_price + '" confirm_quote_fgj_price="' + vm.result[i].fgj_price + '"><i class="bid_table_num">' + (i + 1) + '</i><span class="bid_table_name">' + vm.result[i].seller_company_name + '</span>';
                            if (_proType == "3") {
                                _html += '<span class="bid_table_pay"><span>' + vm.result[i].gj_price + '元<span class="textBlack3 marginR10">(钢筋)</span></span><span>' + vm.result[i].fgj_price + '元<span class="textBlack3">(非钢筋)</span></span></span>';
                            } else {
    ...
    $(".bid_detail_publish").unbind().bind("click", function () {
        ...
        $.ajaxForJson(config.wwwPath + "buyer/Tender/publishEvaluateResultAjax", {
            notice: _notice,
