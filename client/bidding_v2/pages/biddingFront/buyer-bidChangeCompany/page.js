...
$(() => {
    //初始化全部包件数据
    $(".keyUl li").each(function () {
        var main = $(this),
            list = [];
        $.ajaxForJson(config.wwwPath + "/buyer/tender/applyList", {
            package_id: main.attr("packageId")
        }, function (dataObj) {
        ...
        methods: {
            getSupplier() {
                var that = this,
                    inputObj = null,
                    _packageId = "";
                if ($("input[name='packageId']").length > 0) {
                    inputObj = $("input[name='packageId']");
                    _packageId = $("input[name='packageId']").val();

                } else {
                    inputObj = $(".keyUl").find("li.act");
                    _packageId = $(".keyUl").find("li.act").attr("packageId");
                }
    ...
    //切换包件
    $(".bid_body_detail").find(".keyUl a").unbind().bind("click", function () {
        $(".bid_body_detail").find(".keyUl li").removeClass("act");
        $(this).parents("li").addClass("act");
        vmList.getSupplier();
    });

    //邀请供应商
    $(".bid_company_add").unbind().bind("click", function () {
        ...
        _html += '<tr v-for="(item,index) in list"><td>{{index+1}}</td><td>{{item.supplier_name}}</td><td>{{item.typeStr}}</td><td>{{item.supplier_level}}</td><td>{{item.real_name}}</td><td>{{item.user_mobile}}</td><td><a href="javascript:;" class="textBlue" v-if="!item.checked" @click="chageCheck(index)">发送邀请</a><span v-else>已邀请</span></td></tr>';
        ...
        $.dialog({
            ...
            callback: function () {
                ...
                var vm = new Vue({
                    ...
                    mounted() {
                        var inputObj = $("input[name='packageId']").length > 0 ? $("input[name='packageId']") : $(".keyUl").find("li.act");
                    ...
                    methods: {
                        getCompany(currentPage) {
                            var that = this;
                            var _packageId = $("input[name='packageId']").length > 0 ? $("input[name='packageId']").val() : $(".keyUl").find("li.act").attr("packageId");

                            that.loading = true;
                            that.current = currentPage || that.current;
                            $.ajaxForJson(config.wwwPath + "/buyer/tender/getSupplier", {
                                package_id: _packageId,
                                supplier_type: $(".invite_dialog").find(".keyUl").find("li.act").attr("data-value"),
                        ...
                        chageCheck(index) {
                            ...
                            $.ajaxForJson(config.wwwPath + "buyer/alter/supplierAjax", {
                                packageId: _packageId,
                                supplierId: that.list[index].supplier_id
                            }, function (obj) {
                                if (obj.code == 2000) {
                ...
                //切换供应商类型
                $(".invite_dialog").find(".keyUl a").unbind().bind("click", function () {
                    $(".invite_dialog").find(".keyUl").find("li").removeClass("act");
