...
$(() => {
                    ...
                    if ($(".rateTypeDlg-content").find("a.selected").length <= 0) {
                        ...
                        $.ajaxForJson(config.pingbiaoPath + "Evaluation/setScoreMethod", {
                            ...
                            if (dataObj.code == 2000) {
                                ...
                                ScoreMethodId = dataObj.data.data;
                                chooseModelDlg();
                            } else {
                                $.msgTips({
                                    type: "warning",
    ...
    function chooseModelDlg() {
        ...
        for (var i = 0; i < modelList.length; i++) {
            ...
            _dlgHTML += '<tr class="' + _checkClass + '" data-id="' + modelList[i].id + '"><td>' + (i + 1) + '</td><td>' + modelList[i].name + '<i class="iconfont icon-gou"></i></td><td>' + modelList[i].type_title + '</td><td><a class="tdGreen marginR24" target="_blank" href="' + config.pingbiaoPath + 'Evaluation/template/' + modelList[i].id + '.html">预览</a>' + _btnHTML;
            _dlgHTML += '</td></tr>';
        }
    ...
    function confirmModelDlg(tableObj) {
        ...
        for (var i = 0; i < tableObj.content.length; i++) {
            ...
            for (var j = 0; j < tableObj.content[i].content.length; j++) {
                ...
                quanzhongHTML += '<div class="multi_box"><input type="text" class="cjy-input-" value="' + tableObj.content[i].content[j][1] + '"></div>';
            ...
            callback: function () {
                ...
                $(".cjy-confirm-btn,.modelConfirm-save").off().on("click", function () {
                    ...
                    $(".modelConfirm-content").find("tbody.page_list tr").each(function () {
                        if (main.find("td").eq(0).find("input.cjy-input-").val() == "" || main.find("td").eq(1).find("input.cjy-input-").val() == "" || main.find("td").eq(2).find("input.cjy-input-").val() == "") {
                            ajaxKey = false;
                            return false;
                        }
                    });
                    if (ajaxKey) {
                        var _type = main.hasClass("modelConfirm-save") ? 2 : 3;
