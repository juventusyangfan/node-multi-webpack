...
$(() => {
    ...
    (function ($) {
        ...
        $.extend({
            ...
            loadingTips: function (options) {
                ...
                var loadingTipsHtml = '';
                if ($(".cjy-poplayer").length === 0) {
                    loadingTipsHtml = '<div class="cjy-bg" style="height: ' + $(document).outerHeight() + 'px;"></div><div class="cjy-loadingTips"><p class="cjy-loadingTips-inner">';
                } else {
                    loadingTipsHtml = '<div class="cjy-loadingTips"><p class="cjy-loadingTips-inner">';
                }
                if (settings.type === "changbiao") {
                    settings.icon = '<i class="icon-changbiao"></i>';
                }
                loadingTipsHtml += settings.icon + '<span>' + settings.content + '</span></p></div>';
                $("body").eq(0).append(loadingTipsHtml);
                if ($(".cjy-poplayer").length !== 0) {
                    $(".cjy-bg").css("z-index", "101");
                }
                var ele = $(".cjy-loadingTips");
                ele.css({
                    "left": ($(window).outerWidth() - ele.outerWidth()) / 2 + "px",
                    "top": $(document).scrollTop() + ele.outerHeight() + "px",
                    "z-index": 102
    ...
    //切换头部功能
    $(".top_bar_warp").find("a").unbind().bind("click", function () {
        var _index = $(this).parent().index();
        if ($(this).parent().hasClass("noneSetting")) {
            return false;
        } else {
            if ($(this).parent().hasClass("active")) {
                $(this).parent().removeClass("active");
                $(".room_layout").removeClass("room_layout_open");
            } else {
                $(".top_bar_warp").find("li").removeClass("active");
                $(".room_layout").removeClass("room_layout_open");
                $(this).parent().addClass("active");
                if ($(this).hasClass("js_liaotianshi")) {
                    // $.ajaxForJson(config.pingbiaoPath + "chat/join", {}, function (dataObj) {
                    //     if(dataObj.code==2000){
                    //         $(".room_layout").css({
                    //             "height": "790px",
                    //             "padding": "30px 0"
                    ...
                    //     }
                    // });
                    $(".room_layout").eq(_index).addClass("room_layout_open");
                } else if ($(this).hasClass("js_baseInfo")) {
                    $(".room_layout").eq(_index).addClass("room_layout_open");
                } else if ($(this).hasClass("js_group")) {
                    $(".room_layout").eq(_index).addClass("room_layout_open");
                    //切换包件
                    $(".room_layout").eq(_index).find("a.package_cell").unbind().bind("click", function () {
                        var main = $(this),
                            packageId = main.attr("data-id"),
                            tenderId = $(".room_layout").eq(_index).attr("tender-id");
                        $.ajaxForJson(config.pingbiaoPath + "Index/ajaxTenderTeamTab", {
                            tender_id: tenderId,
                            ...
                            if (dataObj.code == 2000) {
                                ...
                                if (dataObj.data) {
                                    ...
                                    for (var i = 0; i < dataObj.data.length; i++) {
                                        if (dataObj.data[i].expert_info) {
                                            _table += '<tr><td>' + (i + 1) + '</td><td>' + dataObj.data[i].expert_info.real_name + '</td><td>' + dataObj.data[i].expert_info.user_mobile + '</td><td>' + dataObj.data[i].expert_info.remarks + '</td></tr>';
                                        }
                                    }
                                    $(".room_layout").eq(_index).find(".material_list tbody").html(_table);
                                }
                            }
                        });
                        return false;
                    });
                } else if ($(this).hasClass("js_rules")) {
                    $(".room_layout").eq(_index).addClass("room_layout_open");
                    $(".room_layout").eq(_index).find("a.package_cell").unbind().bind("click", function () {
                        var main = $(this),
                            packageId = main.attr("package-id"),
                            tenderId = $(".room_layout").eq(_index).attr("tender-id");
                        $.ajaxForJson(config.pingbiaoPath + "Index/ajaxTenderRuleTab", {
                            tender_id: tenderId,
                            package_id: packageId
                        }, function (dataObj) {
                            if (dataObj.code == 2000) {
                                ...
                                if (dataObj.data) {
                                    var _title = dataObj.data.base[0].name,
                                        _type = dataObj.data.ruleTypeTitle;
                                    var _table = '';
                                    for (var i = 0; i < dataObj.data.content.length; i++) {
                                        let _row = dataObj.data.content[i].content.length;
                                        for (var j = 0; j < dataObj.data.content[i].content.length; j++) {
                                            if (j <= 0) {
                                                _table += '<tr><td rowspan="' + _row + '">' + dataObj.data.content[i].name + '</td><td>' + dataObj.data.content[i].content[j][0] + '</td><td>' + dataObj.data.content[i].content[j][1] + '</td><td>' + dataObj.data.content[i].content[j][2] + '</td></tr>';
                                            } else {
                                                _table += '<tr><td>' + dataObj.data.content[i].content[j][0] + '</td><td>' + dataObj.data.content[i].content[j][1] + '</td><td>' + dataObj.data.content[i].content[j][2] + '</td></tr>';
                                            }
                                        }
                                    }
                                    $(".room_layout").eq(_index).find(".setting_list_title p").html(_title);
                                    $(".room_layout").eq(_index).find(".setting_list_title span").html(_type);
                                    $(".room_layout").eq(_index).find(".material_list tbody").html(_table);
                                }
                            }
                        });
