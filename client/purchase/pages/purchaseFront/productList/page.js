...
$(() => {
    var vm = new Vue({
        ...
        data: {
            ...
            city_id: libs.getUrlParam('city_id') || "",//城市id
            cityArr: [],//城市列表
            area_id: libs.getUrlParam('area_id') || 0,//地区id
            areaArr: [],//地区列表
            sort: 0,//排序规则
            product_name: libs.getUrlParam('product_name') || ''//搜索的产品名称
        },
        mounted() {
            this.getProInfo(1);
            this.$nextTick(() => {
        ...
        methods: {
            getProInfo(currentPage) {
                ...
                if (vueObj.product_name != "") {
                    hashVal = 'p=' + vueObj.current + '&product_name=' + vueObj.product_name + '&city_id=' + vueObj.city_id + '&area_id=' + vueObj.area_id + '&sort=' + vueObj.sort
                }
                else {
                    hashVal = 'p=' + vueObj.current + '&fid=' + vueObj.fid + '&pid=' + vueObj.pid + '&cid=' + vueObj.cid + '&brand_id=' + vueObj.brandid + specattrUrl + '&city_id=' + vueObj.city_id + '&area_id=' + vueObj.area_id + '&sort=' + vueObj.sort
                }
                ...
                if (vueObj.product_name != "") {
                    $.get(config.shopPath + 'goods/indexProductNameAjax?' + hashVal, function (dataObj) {
                        ...
                        if (dataObj.code === 2000) {
                            ...
                            if (dataObj.data.warehouse_area.city_info) {
                                vueObj.cityArr = dataObj.data.warehouse_area.city_info;
                                vueObj.$nextTick(() => {
                                    vueObj.city_id = dataObj.data.selected.city_id;
                                    ...
                                    $("select[name='city_code']").unbind().bind("change", function () {
                                        ...
                                        if (vueObj.city_id == 0) {
                                            vueObj.area_id = 0;
                                            vueObj.areaArr = [];
                                            $("select[name='area_code']").attr("value", 0).removeAttr("noRender").html('<option value="0">不限区</option>').initSelect();
                                        }
                                        vueObj.getProInfo(1)
                                    });
                                });
                            }
                            if (dataObj.data.warehouse_area.area_info && vueObj.city_id != 0) {
                                vueObj.areaArr = dataObj.data.warehouse_area.area_info;
                                vueObj.$nextTick(() => {
                                    $("select[name='area_code']").attr("value", vueObj.area_id).removeAttr("noRender").initSelect();
                                    $("select[name='area_code']").unbind().bind("change", function () {
                                        vueObj.area_id = $(this).val();
                                        vueObj.getProInfo(1)
                                    });
                                });
                            }
                    ...
                    $.get(config.shopPath + 'goods/indexAjax?' + hashVal, function (dataObj) {
                        dataObj = JSON.parse(dataObj);
                        if (dataObj.code === 2000) {
                            vueObj.count = dataObj.data.product_list.count;
                            if (vueObj.count == 0) {
