...
$(() => {
    ...
    $(".pDetail_filter").off().on("click", ".pDetail_cell", function () {
        ...
        if (buildAttr.speices_num >= arrUl.length - 1) {
            if (buildAttr.speices_num === arrUl.length - 1) {
                arrUl.eq(buildAttr.voidVal).find("a[data-value]").addClass("disable");
                // 还差一个属性匹配 产品总量显示为所有值
                    ...
                    if (skuMap.hasOwnProperty(key)) {
                        if (reg.test(key)) {
                            if (skuMap[key].real_stock_nums > '0') {
                                ...
                                $("a[data-value='" + arrSpeices[buildAttr.voidVal + 1] + "']").removeClass("disable");
                            }
                ...
                var regArr = [];
                if (buildAttr.speices_num == arrUl.length) {
                    for (var i = 0; i < buildAttr.speices_num; i++) {
                        arrUl.eq(i).find("a[data-value]").addClass("disable");
                        buildAttr = buildUpAttrFuc(arrUl);
                        buildAttr.species_arr[i] = "\\d+:\\d+";
                        let _species_str = ";" + buildAttr.species_arr.join(";") + ";";
                        let reg = new RegExp(_species_str);
                        regArr.push(reg);
                    }
                }
                for (const sku in skuMap) {
                    if (skuMap.hasOwnProperty(sku)) {
                        for (var i = 0; i < regArr.length; i++) {
                            if (regArr[i].test(sku)) {
                                if (skuMap[sku].real_stock_nums > '0') {
                                    // 查找出是哪个产品是空值
                                    var arrSpeices = sku.split(";");
                                    $("a[data-value='" + arrSpeices[i + 1] + "']").removeClass("disable");
                                }
                            }
                        }
                    }
                }
                arrUl.find("a.disable").removeClass("checked");
                for (const sku in skuMap) {
                    if (skuMap.hasOwnProperty(sku)) {
