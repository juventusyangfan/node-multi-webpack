...
$(() => {
    ...
    (function ($) {
        ...
        $.extend({
            ...
            messageDialog: function messageDialog(options) {
                ...
                switch (settings.icon) {
                    ...
                    case "error":
                        ...
                        break;
                    case "warning":
                        iClass = 'icon-gantanhao';
                        break;
                    default:
                        break;
                }
                var dialogHTML = '<div class="cjy-msg-box"><i class="iconfont ' + iClass + '"></i><p>' + settings.content + '</p></div>';
                $.dialog({
            ...
            cueDialog: function cueDialog(options) {
                ...
                $.extend(settings, options || {});
                var dialogHTML = '<div class="cjy-cue-box"><i class="iconfont icon-gantanhao"></i><p class="cue-main">' + settings.content + '</p>';
                if (settings.hint !== "") {
                    dialogHTML += '<p class="cue-hint">' + settings.hint + '</p>';
                }
                dialogHTML += '</div>';
                $.dialog({
                    title: settings.title,
                ...
                $.dialog({
                    ...
                    callback: function () {
                        ...
                        $(".sqhy-btn").off().on("click", function () {
                            var main = $(this);
                            $.ajaxForJson(config.accountPath + 'addMember', {
    ...
    (function ($) {
        $.extend({
            ...
            ajaxPromise: function (requestUrl, requestData) {
            ...
            initCartFuc: function () {
                $.ajaxForJson('/Buyer/Shop/ajaxCartList', null, function (dataobj) {
                    if (dataobj.code === 2000) {
                        // 判断对象类型
                        var returnStr = Object.prototype.toString.call(dataobj.data);
                        var findSpace = returnStr.indexOf(' ') + 1;
                        var objType = returnStr.slice(findSpace, returnStr.length - 1);
                        if (objType !== "Array") {
                            var list = null;
                            var listNum = 0;
                            var totalCost = 0;
                            var listHtml = '<div class="cart-list"><ul>';
                            for (const key in dataobj.data) {
                                if (dataobj.data.hasOwnProperty(key)) {
                                    list = dataobj.data[key].list;
                                    for (let i = 0; i < list.length; i++) {
                                        if (parseInt(list[i].goods_info.status) === 0) {
                                            listHtml += '<li class="cart-item off-shelf" data-proid="' + list[i].id + '"><div class="cart-inner clearfix"><figure class="cart-photo"><img src="' + config.filePath + list[i].goods_info.main_img + '" /></figure><div class="cart-intro"><a class="cart-link" href="javascript:;">' + list[i].goods_info.product_name + '</a><div class="cart-info"><span class="shelf-state">下架</span><a class="cart-del" href="javascript:;">删除</a></div></div></div></li>';
                                        } else {
                                            totalCost += parseFloat(list[i].goods_info.price) * parseFloat(list[i].amount);
                                            listHtml += '<li class="cart-item" data-proid="' + list[i].id + '"><div class="cart-inner clearfix"><figure class="cart-photo"><img src="' + config.filePath + list[i].goods_info.main_img + '" /></figure><div class="cart-intro"><a class="cart-link" href="/goods/detail.html?product_id=' + list[i].goods_info.product_id + '">' + list[i].goods_info.product_name + '</a><div class="cart-info"><p class="cart-data"><strong>￥' + list[i].goods_info.price + '</strong>&nbsp;×&nbsp;<span>' + list[i].amount + '</span></p><a class="cart-del" href="javascript:;">删除</a></div></div></div></li>';
                                        }
                                        listNum += 1;
                                    }
                                }
                            ...
                            listHtml += '</ul></div>';
                            $(".cart-container").html(listHtml);
                            $(".goods-amount").html(listNum);
                            $(".cart_num").html(listNum);
                            $(".add-up-box .highlight").eq(0).html(listNum);
                            $(".add-up-box .highlight").eq(1).html("￥" + totalCost.toFixed(2));
                            // 删除
                            $(".cart-del").off().on("click", function () {
                                var curDel = $(this);
                                $.ajaxForJson('/Buyer/Shop/ajaxDelCart', {
                                    id: curDel.parents(".cart-item").attr("data-proid")
                                }, function (json) {
                                    if (json.code === 2000) {
                                        curDel.parents(".cart-item").remove();
                                        if ($(".cart-item").length <= 0) {
                                            $(".cart-container").html('<div class="no-result"><div class="noresult-goods"><div><figure class="goods-default"><img src="' + config.staticPath + 'purchase/static/img/722e6288eece7320aa5a5d5af1badd38.png"><figcaption>购物车还没有商品哦快去选购吧~</figcaption></figure><a class="go_home" href="' + config.shopPath + '">去首页</a></div></div></div>');
                                            $(".add-up-box .highlight").eq(1).html("￥0.00");
                                        } else {
                                            var singlePrice = singleNum = singleTotal = 0;
                                            for (let j = 0; j < $(".cart-item").length; j++) {
                                                if ($(".cart-item").eq(j).hasClass("off-shelf")) {
                                                    continue;
                                                }
                                                singleNum = parseFloat($(".cart-item").eq(j).find("span").html());
                                                singlePrice = parseFloat($(".cart-item").eq(j).find("strong").html().replace(/￥/, ""));
                                                singleTotal += (singleNum * singlePrice);
                                            }
                                            $(".add-up-box .highlight").eq(1).html("￥" + singleTotal.toFixed(2));
                                        }
                                        if (!curDel.hasClass("off-shelf")) {
                                            var amountStr = parseInt($(".goods-amount").html()) - 1;
                                            $(".goods-amount").html(amountStr);
                                            $(".add-up-box .highlight").eq(0).html(amountStr);
                                            $(".cart_num").html(amountStr);
                                        }
                                    }
                                });
                            });
                        ...
                        // 去购物车结算
                        $(".settle-bill").off().on("click", function () {
                            if ($(".cart-item").length <= 0) {
                                $.msgTips({
                                    type: "warning",
                                    content: "购物车是空的哦~"
    ...
    //文件上传Uppy
    $.fn.uppyUpload = function (uploadObj, successCallback, options) {
        var token = "",
            tokenNum = 0,
            md5Key = true;
        ...
        const uppyOne = new Uppy({
            ...
            onBeforeFileAdded: (currentFile, files) => {
                md5Key = true;
                var _arr = currentFile.name.split("."),
                    _ext = _arr[_arr.length - 1].toLowerCase();
                if (currentFile.data.size >= 500 * 1024 * 1024) {
                    $.msgTips({
                        type: "warning",
                        content: "上传文件过大"
                    });
                    return false;
                } else if (settings.extArr != "all" && settings.extArr.indexOf(_ext) <= -1) {
                    $.msgTips({
                        type: "warning",
                        content: "上传文件格式不正确"
                    });
                    ...
                    bmf.md5(
                        ...
                        (err, md5) => {
                            if (md5Key) {
                                ...
                                $.ajax({
                                    url: config.upload + "group1/upload?output=json&md5=" + md5 + "&auth_token=" + token,
                                    async: false,
                                    dataType: "json",
                                    success: function (data) {
                                        if (data.md5 != "") {
                                            uppyOne.cancelAll();
                                            successCallback(currentFile.data.name, data.path);
                                        }
                                        return false;
                                    },
                                    error: function () {
        ...
        uppyOne.use(FileInput, {
            ...
            .use(Tus, {
                endpoint: config.upload + 'group1/big/upload/'
            })
            .use(StatusBar, {
                target: settings.processCon,
        ...
        uppyOne.on('upload-error', (file, error, response) => {
            $.msgTips({
                type: "warning",
                content: "上传错误，请及时联系我们进行解决！"
            });
        });
        ...
        uppyOne.on('complete', (result) => {
            ...
            for (var i = 0; i < result.successful.length; i++) {
                md5Key = false;
                let _name = result.successful[i].name,
                    _url = "",
                    _urlArr = result.successful[i].uploadURL.split("/"),
                    _md5 = _urlArr[_urlArr.length - 1];
                if (tokenNum == 0) {
                    getToken();
                }
                let _ajaxKey = true,
                    _ajaxNum = 1;
                let _ajaxTimer = setInterval(function () {
                    if (_ajaxNum > 5) {
                        clearInterval(_ajaxTimer);
                    } else {
                        if (_ajaxKey) {
                            $.ajax({
                                url: config.upload + "group1/upload?output=json&md5=" + _md5 + "&auth_token=" + token,
                                async: false,
                                dataType: "json",
                                success: function (data) {
                                    clearInterval(_ajaxTimer);
    ...
    (function ($) {
        ...
        function getDate(string, format) {
            ...
            if (validDate(string, format)) {
                ...
                if (reDate.getDate() == day) return reDate;
                else return tmpnow;
            } else return tmpnow;
        ...
        lhgcalendar.fn = lhgcalendar.prototype = {
            ...
            _init: function _init(config) {
                ...
                if (!config.btnBar) DOM.foot[0].style.display = 'none';
                else DOM.foot[0].style.display = '';
                ...
                if (inpVal.length > 0) date = getDate(inpVal, config.format);
                else date = new Date();
                ...
                for (var i = 0; i < 6; i++) {
                    ...
                    for (var j = 0; j < 7; j++) {
                        ...
                        if (n < firstDay || n >= curMonth + firstDay || opt.minDate && opt.minDate > curDateStr || opt.maxDate && opt.maxDate < curDateStr || opt.disWeek && opt.disWeek.indexOf(j) >= 0) {
                            ...
                            if (tmpre) that._setCell(cell, arrDate[n]);
            ...
            _drawList: function _drawList(val, arr) {
                ...
                for (var i = 0; i < 4; i++) {
                    ...
                    for (var j = 0; j < 3; j++) {
                        ...
                        if (arr) cell.firstChild[expando + 'M'] = val;
                        else cell.firstChild[expando + 'Y'] = val;
            ...
            _addEvent: function _addEvent() {
                ...
                DOM.wrap.bind('click', function (evt) {
                    ...
                    if (target[expando + 'D']) that._setDate(target[expando + 'D']);
                    else if (target === DOM.pm[0] || target.parentNode === DOM.pm[0]) that._draw(new Date(that.year, that.month - 2), that.day);
                    else if (target === DOM.nm[0] || target.parentNode === DOM.nm[0]) that._draw(new Date(that.year, that.month), that.day);
                    else if (target === DOM.py[0] || target.parentNode === DOM.py[0]) that._draw(new Date(that.year - 1, that.month - 1), that.day);
                    else if (target === DOM.ny[0] || target.parentNode === DOM.ny[0]) that._draw(new Date(that.year + 1, that.month - 1), that.day);
    ...
    //             let data = canvas.toDataURL('image/jpeg'); // 将canvas转成dataURL，不通的图片类型data的大小差别较大，也可以设置图片的质量，以控制生成文件的大小
    //             let createPdf = (function (canvas, doc, imgData) {  //创建PDF
    //                 /*
    //                   通过canvas生成PDF
    //                   @can    {object}     canvas对象
    ...
    //                 let [ph, ch, pw, cw] = [doc.internal.pageSize.getHeight(), canvas.height, doc.internal.pageSize.getWidth(), canvas.width]
    ...
    //             createPdf(data, 0);
    //
    //             var fileData = doc.output('datauristring')
    //
    //             if (callback) {
    //                 callback(fileData);
    //             }
    ...
    $.fn.ajxForPage = function (reqUrl, reqData, callback) {
        $(this).each(function () {
            var main = $(this);
            var curPage = 1;
            var size = main.attr("size"),
                total = main.attr("total"),
    ...
    Vue.component('vuePage', {
        ...
        props: ['count', 'limit', 'cpage'],
        computed: {
            totalPage() {
                var that = this;
                that.currentPage = that.cpage || 1;
    ...
    $(".top_menu_right .top_search_btn").unbind().bind("click", function () {
        if ($(".top_menu_right .top_search_sel > span").html() == "产品") {
            window.location.href = config.shopPath + "goods/index.html?product_name=" + encodeURIComponent($(".top_menu_right input.top_search_input").val());
        } else {
            window.location.href = config.shopPath + "companylist.html?company_name=" + encodeURIComponent($(".top_menu_right input.top_search_input").val());
        }
        return false;
    });
    ...
    $(".top_menu_right .mall_search_btn").unbind().bind("click", function () {
        window.location.href = config.shopPath + "goods/index.html?product_name=" + encodeURIComponent($(".top_menu_right input.shop_search_input").val());
        return false;
    });
    //搜索本店
    $(".top_menu_right .shop_search_btn").unbind().bind("click", function () {
        var companyId = libs.getUrlParam('company_id');
        window.location.href = config.shopPath + "company/index.html?company_id=" + companyId + "&product_name=" + encodeURIComponent($(".top_menu_right input.shop_search_input").val());
        return false;
    });
    $(".top_menu_right input.shop_search_input").unbind().bind("keypress", function (e) {
        if (e.keyCode == "13") {
            $(".top_menu_right .shop_search_btn").trigger("click");
        }
    });
    ...
    // 展开收起购物车
    $(".shop-cart,.close-cart").off().on("click", function () {
        var main = $(".shop-cart");
        if (main.hasClass("cart-on")) {
            main.removeClass("cart-on");
            $(".toolbar-menu").css("right", "0px");
            $(".cart-box").css("right", "-281px");
        } else {
            main.addClass("cart-on");
            $(".toolbar-menu").css("right", "281px");
            $(".cart-box").css("right", "0px");
        }
    });
    $(document).on("click", function (e) {
        var _con = $('.shop-cart,.cart-box'); // 设置目标区域
        if (!_con.is(e.target) && _con.has(e.target).length === 0) {
            $(".shop-cart").removeClass("cart-on");
            $(".toolbar-menu").css("right", "0px");
            $(".cart-box").css("right", "-281px");
        }
    });
    ...
    $(".website_location").find("a").unbind().bind("click", function () {
        var main = $(this),
            cityId = main.attr("city_id");
        if (!main.hasClass("act")) {
            $.ajaxForJson(config.shopPath + "/changeCity", {
                city_id: cityId
            }, function (dataObj) {
                if (dataObj.code == 2000) {
    ...
    // 下拉菜单
    $.ajaxForJson(config.shopPath + "/catalogTree", null, function (dataObj) {
        var liHTML = '',
            itemHTML = '';
        if (dataObj.code == 2000) {
            var liHeight = 480 / dataObj.data.length;
            for (var i = 0; i < dataObj.data.length; i++) {
                liHTML += '<li class="service_cat" data-index="' + i + '" style="height:' + liHeight + 'px;line-height:' + liHeight + 'px;"><a href="' + config.shopPath + 'goods/index.html?fid=' + dataObj.data[i].value + '" target="_blank"><i class="left_icon iconfont ' + dataObj.data[i].icon + '"></i><span class="ellipsis">' + dataObj.data[i].label + '</span><i class="iconfont icon-youjiantou"></i></a></li>';
                itemHTML += '<div class="service_item">';
                if (dataObj.data[i].children) {
                    for (var j = 0; j < dataObj.data[i].children.length; j++) {
                        itemHTML += '<dl class="service_panel clearfix"><dt class="service_panel_dt"><a href="' + config.shopPath + 'goods/index.html?fid=' + dataObj.data[i].value + '&pid=' + dataObj.data[i].children[j].value + '" target="_blank" title="' + dataObj.data[i].children[j].label + '"><i class="iconfont icon-youjiantou"></i>' + dataObj.data[i].children[j].label + '</a></dt><dd class="service_panel_dd">';
                        if (dataObj.data[i].children[j].children) {
                            for (var k = 0; k < dataObj.data[i].children[j].children.length; k++) {
                                itemHTML += '<a href="' + config.shopPath + 'goods/index.html?fid=' + dataObj.data[i].value + '&pid=' + dataObj.data[i].children[j].value + '&cid=' + dataObj.data[i].children[j].children[k].value + '" target="_blank">' + dataObj.data[i].children[j].children[k].label + '</a>'
                            }
                        }
                        itemHTML += '</dd></dl>';
                    }
            ...
            $(".top_service").find("ul.service_bd").html(liHTML);
            $(".top_service").find(".service_float").html(itemHTML);

            $(".service_bd").off().on("mouseover", ".service_cat", function () {
                $(".service_bd").find(".service_cat").removeClass("open");
                $(this).addClass("open");
                var curIndex = $(this).attr("data-index");
                $(".service_float").css("display", "block");
                $(".service_item").css("display", "none");
                $(".service_item").eq(curIndex).css("display", "block");
            });
            $(".top_service").off().on("mouseout", function (event) {
                // 如果是鼠标移入子元素则取消mouseout事件
                ...
                $(".service_bd").find(".service_cat").removeClass("open");
                $(".service_float").css("display", "none");
            });
        }
    });
});