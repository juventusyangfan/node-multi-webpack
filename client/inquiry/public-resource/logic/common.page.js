...
$(() => {
    ...
    (function ($) {
        ...
        $.extend({
            ...
            loginDialog: function loginDialog() {
                ...
                dialogCon += '<div class="login_item"><i class="iconfont icon-ren user_icon"></i><input type="text" name="user_mobile" placeholder="请输入手机号"></div>';
            ...
            loadingDialog:function(){
                ...
                showPhotoHtml += '<div class="cjy-show-photo" style="left: ' + ($(window).outerWidth() - 60) / 2 + 'px;top: ' + ($(document).scrollTop() + ($(window).outerHeight() - 60) / 2) + 'px"><span class="cjy-loading-img"></span></div>';
    ...
    $.fn.initSupplier = function () {
        ...
        this.each(function (i) {
            var main = $(this);
            main.extend({
                area: main.find('.notify_container').on('mouseover', function (e) {
                    main.ishover = true;
                ...
                timer: null,
                    ...
                    var curItem = main.find('li').eq(0);
                    curItem.clone(true).appendTo(main.wrap);
                    main.wrap.css('top', '0px');
                    curItem.remove();
                    main.timer = setTimeout(main.run, 5000);
                },
                run: function run() {
                    if (main.find('li').length > 1 && !main.ishover) {
                        main.wrap.stop(true, true);
                        main.wrap.animate({
                            'top': '-40px'
                        }, 500, main.done);
                    }
    ...
    $(".top_menu_right .top_search_btn").unbind().bind("click", function () {
        if ($(".top_menu_right .top_search_sel > span").html() == "询价单") {
            window.location.href = config.inquiryPath + "?title=" + encodeURIComponent($(".top_menu_right input.top_search_input").val());
        } else if ($(".top_menu_right .top_search_sel > span").html() == "公司") {
            window.location.href = config.inquiryPath + "?company_name=" + encodeURIComponent($(".top_menu_right input.top_search_input").val());
        } else {
            window.location.href = config.inquiryPath + "?product_name=" + encodeURIComponent($(".top_menu_right input.top_search_input").val());
        }
        return false;
    });
