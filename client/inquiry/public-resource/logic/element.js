    ...
    $.fn.extend({
        ...
        initMultiSelect: function (callback) {
            $(this).each(function () {
                ...
                var _first = multi.attr("data-first") || "0",
                    _parent = multi.attr("data-parent") || "0",
                    _id = multi.attr("data-id") || "0",
                    _name = multi.attr("data-name") || "";

                var multiHTML = '<div class="cjy-multi"><div class="cjy-multi-title"><input type="hidden" name="inquiry_goods[first_category_id][]" value="' + _first + '"><input type="hidden" name="inquiry_goods[parent_category_id][]" value="' + _parent + '"><input type="hidden" name="inquiry_goods[category_id][]" value="' + _id + '"><input type="text" name="inquiry_goods[product_name][]" placeholder="请选择" readonly="" class="cjy-multi-input" value="' + _name + '"><i class="iconfont icon-xiajiantou"></i></div><div class="cjy-multi-content"></div></div>';
                ...
                multi.find(".cjy-multi-content").off().on("click", "li a", function () {
                    var main = $(this);
                    var index = main.parents("ul").index();
                    ...
                    switch (index) {
                        case 0:
                            multi.find("input[name='inquiry_goods[first_category_id][]']").val(id);
                            break;
                        case 1:
                            multi.find("input[name='inquiry_goods[parent_category_id][]']").val(id);
                            break;
                        case 2:
                            multi.find("input[name='inquiry_goods[category_id][]']").val(id);
                            break;
                    }
                ...
                function getCata(id) {
                    ...
                    $.ajax({
                        type: "POST",
                        async: false,
                        url: config.inquiryPath + "/catlist",
                        data: {
                            id: cataId
                        },
                        dataType: "json",
                        success: function (dataObj) {
                            for (var i = 0; i < dataObj.data.length; i++) {
                                liHTML += '<li><a href="javascript:;" data-id="' + dataObj.data[i].code + '" title="' + dataObj.data[i].name + '">' + dataObj.data[i].name + '</a></li>'
                            }
                        }
                    });
                    return liHTML;
